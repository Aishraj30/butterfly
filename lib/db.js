...
export async function connectDB() {
    ...
    if (!cached.promise) {
        const opts = {
            ...
            family: 4, // Keep IPv4 if the cluster is strictly IPv4, but increase timeouts
            retryWrites: true,
            writeConcern: {
                w: 'majority'
            }
    ...
    try {
        ...
        if (e.message.includes('SSL routines')) {
            console.error("💡 TIP: This SSL error often means your IP is not whitelisted in MongoDB Atlas or there is a TLS version mismatch.");
        }
        throw e;
    }
